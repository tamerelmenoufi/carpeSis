<?php
    ...
    if($_POST['acao'] == 'salvar'){
        ...
        if(!$_POST['codigo']){
            $attr[] = "categoria = '" . $_SESSION['categoria'] . "'";
        }

        $attr = implode(', ', $attr);

        if($_POST['codigo']){
            $query = "update sub_categorias set {$attr} where codigo = '{$_POST['codigo']}'";
            mysqli_query($con, $query);
            $codigo = $_POST['codigo'];
        }else{
            $query = "insert into sub_categorias set {$attr}";
            mysqli_query($con, $query);
    ...
    $query = "select * from sub_categorias where codigo = '{$_POST['codigo']}'";
...
<h4 class="Titulo<?=$md5?>">Registro de sub_categorias</h4>
    <form id="form-<?= $md5 ?>">
        <div class="row">
            <div class="col">
        ...
        <div class="row">
            <div class="col">
                <div style="display:flex; justify-content:end">
                    ...
                    <input type="hidden" id="categoria" value="<?=$_SESSION['categoria']?>" />
                </div>
            </div>
        </div>
    </form>
